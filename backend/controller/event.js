...
const istDateTimeString = now.tz(istTimeZone).format('YYYY-MM-DDTHH:mm:ss.SSSZ');
const utcDateTimeString = now.tz(istTimeZone).utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]');
...
exports.createEvent=async (req,res)=>{
   try{
    const {limit,title,description,venue,state,date,startat,endat}=req.body;
    const user=req.user;
    ...
    if(endTime<=startTime){
        return res.status(400).send({"msg":"End time should be greater than start time"})
    }
...
exports.eventbycity=async(req,res)=>{
    try{
        ...
        const allevent=await Event.find({state:{$regex:city},date:{$gt:timeconvertor()}}).sort({date:1,startat:1});
        res.send(allevent);
    }
    catch(err){
...
exports.bookticket=async(req,res)=>{ 
  try{
    ...
    return res.send({"msg":"You have been registered successfully kindy wait for the conformation"})
  }
  catch(err){
...
exports.conformticket=async (req,res)=>{
    // try{
     ...
     if(mainevent.organisedby!==user._id.toString()){
        return res.status(400).send({"msg":"Access denied"})
     }
     if(eventbypass.status==="rejected" || eventbypass.status==="accepted"){
        return res.status(401).send({"msg":`Already ${eventbypass.status}`})
     }

     if(status==='accepted' && mainevent.partcipants.length<mainevent.limit){
